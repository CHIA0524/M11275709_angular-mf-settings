...
import { AfterViewInit, ChangeDetectorRef, Component, ElementRef, NgZone, OnDestroy, ViewChild, computed, inject, signal } from '@angular/core';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, RouterModule, MatButtonModule, MatCardModule, MatFormFieldModule, MatInputModule, MatSelectModule, MatSlideToggleModule],
...
export class SettingsWorkspaceComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    // 確保 component 已掛載到 CD tree（federated lazy load 有時會 detach）
    this.cdr.reattach();
    // 在本次 Angular CD task 結束後再觸發一輪 tick，讓 Material CVA 有時間 register
    this.scheduleAppTick();
  ...
  private scheduleAppTick(): void {
    // 使用 cdr.detectChanges() 直接對此元件樹觸發 CD，
    // 確保 Material 元件的 CVA 在 federated lazy load 後正確初始化
    setTimeout(() => this.ngZone.run(() => this.cdr.detectChanges()), 0);
  }
